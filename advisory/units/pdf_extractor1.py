...
_RECOMMENDATION_MARKERS = re.compile(
    r'(?:RECOMMENDATION|FERTILIZER\s+RECOMMENDATION|FERTILISER\s+RECOMMENDATION'
    r'|SUGGESTED\s+(?:DOSE|APPLICATION)|DOSAGE\s+RECOMMENDATION'
    r'|TREATMENT\s+SCHEDULE|NUTRIENT\s+MANAGEMENT\s+PLAN'
    r'|SUGGESTED\s+FERTILIZER|FERTILIZER\s+SCHEDULE)',
    re.IGNORECASE,
)
...
_REGEX_PATTERNS: list[tuple[str, re.Pattern]] = [
    ...
    ("EC", re.compile(
        r'(?:electrical\s+conductivity|EC)\s*(?:dS/m|mS/cm)?[^\n]{0,20}?(\d+\.?\d*)',
        re.IGNORECASE,
    )),
]
...
# FIX: Broadened texture pattern — original missed "Sandy clay loam" and
# other multi-word textures because the alternation was too narrow.
# Now captures any 2–5 word texture description after the label.
_SOIL_TYPE_RE = re.compile(
    r'(?:soil\s+type|soil\s+texture|texture)[^\n]{0,30}?[:\s]+'
    r'((?:sandy\s+clay\s+loam|sandy\s+clay|clay\s+loam|silty\s+clay\s+loam'
    r'|silty\s+clay|silt\s+loam|sandy\s+loam|loamy\s+sand'
    r'|clay|loam|sandy|silty|silt))',
    re.IGNORECASE,
)

# NEW: CEC (Cation Exchange Capacity)
_CEC_RE = re.compile(
    r'(?:cation\s+exchange\s+capacity|CEC)[^\n]{0,30}?(\d+\.?\d*)',
    re.IGNORECASE,
)

# NEW: Base Saturation
_BASE_SAT_RE = re.compile(
    r'(?:base\s+saturation)[^\n]{0,30}?(\d+\.?\d*)',
    re.IGNORECASE,
)

# NEW: SAR (Sodium Adsorption Ratio)
_SAR_RE = re.compile(
    r'(?:sodium\s+adsorption\s+ratio|SAR)[^\n]{0,30}?(\d+\.?\d*)',
    re.IGNORECASE,
)
...
def extract_soil_from_pdf(file_bytes: bytes) -> tuple[dict, dict, str, dict, str, str]:
    ...
    # NEW: capture CEC, Base Saturation, SAR
    cec_m = _CEC_RE.search(raw_text)
    if cec_m:
        try:
            all_extracted["CEC"] = float(cec_m.group(1))
        except ValueError:
            pass
    bs_m = _BASE_SAT_RE.search(raw_text)
    if bs_m:
        try:
            all_extracted["BaseSat"] = float(bs_m.group(1))
        except ValueError:
            pass
    sar_m = _SAR_RE.search(raw_text)
    if sar_m:
        try:
            all_extracted["SAR"] = float(sar_m.group(1))
        except ValueError:
            pass
